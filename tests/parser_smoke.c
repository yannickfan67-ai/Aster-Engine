...
int main(void){
    AsterDocument d;
    const char *html="<html><head><title>Aster selectors</title><style>p{color:#123456}.notice{font-weight:bold}#hero{color:#654321}p.notice{font-size:24px}.accent,#other{text-decoration:underline}.notice.accent{color:#0a0b0c}p.notice.accent{font-size:24px}#gone{display:none}</style></head><body><p id='hero' class='notice accent'>Hello CSS</p><p id='other'>Other</p><p class='notice accent'>Compound</p><p id='gone'>hidden id</p><img src='test.bmp' alt='demo' width='64' height='32'></body></html>";
    if(!aster_parse_html(&d,html))return 1;
    aster_layout(&d,320);
    if(d.css_count<9)return 2;
    int saw_hero=0,saw_other=0,saw_compound=0,saw_image=0;
    for(unsigned i=0;i<d.paint_count;i++){
        const AsterPaintItem*p=&d.paint[i];
    ...
    if(!saw_hero||!saw_other||!saw_compound||!saw_image)return 3;
