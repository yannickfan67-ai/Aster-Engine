...
uint32_t gfx_text(uint32_t x,uint32_t y,const char*s,uint32_t rgb,uint32_t scale){
    if(g_ready&&(g_ops.capabilities&ASTER_PLATFORM_CAP_TEXT)&&g_ops.draw_text)
        return g_ops.draw_text(g_context,x,y,s,rgb,scale);
    uint32_t n=0;while(s&&s[n])n++;return n*11u*(scale?scale:1u);
}
uint32_t gfx_display_text(uint32_t x,uint32_t y,const char*s,uint32_t rgb){
    if(g_ready&&(g_ops.capabilities&ASTER_PLATFORM_CAP_DISPLAY_TEXT)&&g_ops.draw_display_text)
        return g_ops.draw_display_text(g_context,x,y,s,rgb);
    return gfx_text(x,y,s,rgb,2);
}
void gfx_line_h(uint32_t x,uint32_t y,uint32_t w,uint32_t rgb){
    if(g_ready&&(g_ops.capabilities&ASTER_PLATFORM_CAP_LINES)&&g_ops.draw_line_h)
        g_ops.draw_line_h(g_context,x,y,w,rgb);
}
