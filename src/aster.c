...
static void layout_walk(AsterDocument*d,int node,int *cx,int *cy,int left,int width){
    ...
    if(block&&*cx!=left){*cx=left;*cy+=24;}if(tag==ASTER_TAG_H1||tag==ASTER_TAG_H2||tag==ASTER_TAG_P||tag==ASTER_TAG_DIV||tag==ASTER_TAG_LI)*cy+=8;if(tag==ASTER_TAG_LI){*cx=left+22;}
